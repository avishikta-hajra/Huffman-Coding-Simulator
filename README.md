...
This project is built on a split architecture : **C++ handles the algorithmic logic**, while **React handles the state tracking and visual rendering**.
...
* **`src/index.css` :** Custom styling using CSS pseudo-elements to draw the tree branches (connectors and stems) dynamically without needing HTML `<canvas>`.
...
8. **Add Labels :** The user clicks "Add Label". React recursively passes a `path` string prop down the completed tree (`"0"` for the left branch, `"1"` for the right branch). The original leaf nodes (like `b` or `c`) receive their final binary string (e.g., `000` or `11`) and display it.
